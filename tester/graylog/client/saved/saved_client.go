...
type ClientService interface {
	SearchRelative(params *SearchRelativeParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*SearchRelativeOK, error)
...
func (a *Client) SearchRelative(params *SearchRelativeParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*SearchRelativeOK, error) {
	...
	success, ok := result.(*SearchRelativeOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for searchRelative: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
